...
项目重点解决三个问题：保留 PDF 页码和 Word 段落位置，兼顾关键词与同义表达检索，以及避免模型把不存在的原文写成引用。资料不够、检索无结果和模型输出异常会显示为不同状态，方便判断下一步该补文档、换问法还是重试。

**在线体验：** [在 Streamlit Cloud 打开知页](https://ai-knowledge-assistant-qujnzjhmdavaeachd75gvp.streamlit.app/)
...
打开内置的虚构员工手册后，可以先提问“正式员工每年有几天年假”，再追问“那试用期员工呢”，最后询问文档没有写到的午餐政策。这组问题依次覆盖原文定位、多轮追问和资料不足处理。
